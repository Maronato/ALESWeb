...
def login_successful(code, request, user, key):
    """Login Successful
    ...
    new = FacebookUser.create_or_update(social_id, access_token, token_expires, first_name, name, link, user)

    # Try to login the user
    if new is None:
        messages.add_message(request, messages.ERROR, 'Você precisa estar matriculado(a) para entrar!')
    elif new.user.is_active:
